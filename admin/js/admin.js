...
function cardImageError(el) {
    el.parentElement.outerHTML = '<div class="card-no-image"><i class="fas fa-image"></i><span>No Image</span></div>';
}
...
function renderWithNesting(type, data, gridId) {
    ...
    var useGrouping = !search && !category && (type === 'products' || type === 'services');
    ...
    if (useGrouping) {
        ...
        data.forEach(function (item, index) {
            ...
            groups[cat].push({ item: item, index: index });
        });

        groupOrder.forEach(function (cat) {
            var catId = cat.replace(/[^a-zA-Z0-9]/g, '-').toLowerCase();
            html += '<div class="category-group" data-category="' + escapeHTML(cat) + '">';
            html += '<div class="category-group-header" onclick="toggleCategoryGroup(this)">';
            html += '<h3><i class="fas fa-chevron-down category-chevron"></i> ' + escapeHTML(cat) +
                ' <span class="category-count">' + groups[cat].length + '</span></h3>';
            html += '</div>';
            html += '<div class="items-grid category-grid" id="cat-grid-' + catId + '">';
            groups[cat].forEach(function (entry) {
                html += renderItemCard(type, entry.item, entry.index);
                var children = entry.item.subServices || entry.item.subProducts;
...
function renderItemCard(type, item, index, isChild) {
    ...
    if (type === 'testimonials' && item.logo) {
        if (item.logo.startsWith('http') || item.logo.startsWith('data:') || item.logo.startsWith('assets/')) {
            imageUrl = item.logo;
        } else {
    ...
    var imageHtml = imageUrl ?
        ...
        '<img loading="lazy" src="' + escapeHTML(imageUrl) + '" alt="' + escapeHTML(title) + '" onerror="cardImageError(this)">' +
        '</div>' : '<div class="card-no-image"><i class="fas fa-image"></i><span>No Image</span></div>';
...
function renderClients() {
    ...
    groupOrder.forEach(function (cat) {
        ...
        html += '<h3><i class="fas fa-chevron-down category-chevron"></i> ' + escapeHTML(cat) +
            ' <span class="category-count">' + groups[cat].length + '</span></h3>';
        html += '</div>';
        html += '<div class="items-grid category-grid" id="cat-grid-clients-' + catId + '">';
        groups[cat].forEach(function (entry) {
            html += renderItemCard('clients', entry.item, entry.index);
        });
        html += '</div></div>';
    });
...
function renderLocations() {
    ...
    grid.innerHTML = '<div class="items-grid">' + locations.map(function (item, index) {
        return '<div class="item-card">' +
            ...
            '</div></div>' +
            '<p><strong>Address:</strong><br>' + escapeHTML(item.address).replace(/&lt;br\s*\/?&gt;/gi, '<br>') + '</p>' +
            '<div class="item-card-actions">' +
...
function normalizeLinkedInEmbedUrl(url) {
    url = (url || '').trim();
    if (!url) return '';
    if (url.includes('linkedin.com/embed/')) return url;
    // Try to convert a regular post/feed URL by extracting the URN
    var urnMatch = url.match(/urn:li:\w+:\d+/);
    if (urnMatch) return 'https://www.linkedin.com/embed/feed/update/' + urnMatch[0];
    return url;
}
    ...
    for (var t = 0; t < types.length; t++) {
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            if (img && !img.startsWith('http') && !img.startsWith('data:')) {
                try {
                    var fetchPath = img.startsWith('/') ? img : '/' + img;
                    var res = await fetch(fetchPath, { method: 'HEAD' });
                    if (!res.ok) {
...
function parseAboutPageContent(content) {
    ...
    if (match) {
        try {
            var raw = match[1].replace(/;+$/, '').replace(/"[^"\\]*(?:\\.[^"\\]*)*"|\/\/[^\n]*/g, function(m) { return m.charAt(0) === '"' ? m : ''; }).replace(/,\s*([\]}])/g, '$1');
...
function saveNavigation() {
    ...
    for (var i = 0; i < links.length; i++) {
        ...
        links[i].customClass = (document.getElementById('nav-custom-class-' + i) || {}).value || '';
        links[i].external = !!((document.getElementById('nav-external-' + i) || {}).checked);
        links[i].key = (links[i].href || '').replace(/^#/, '') || '/';
        if (!links[i].shortLabel) delete links[i].shortLabel;
        if (!links[i].customClass) delete links[i].customClass;
        if (!links[i].external) delete links[i].external;
...
function parseLayoutData(content) {
    ...
    if (matchMega) {
        try {
            var rawMega = matchMega[1].replace(/"[^"\\]*(?:\\.[^"\\]*)*"|\/\/[^\n]*/g, function(m) { return m.charAt(0) === '"' ? m : ''; }).replace(/,\s*([\]}])/g, '$1');
    ...
    if (match3) {
        try {
            var raw3 = match3[1].replace(/;+$/, '').replace(/"[^"\\]*(?:\\.[^"\\]*)*"|\/\/[^\n]*/g, function(m) { return m.charAt(0) === '"' ? m : ''; }).replace(/,\s*([\]}])/g, '$1');
