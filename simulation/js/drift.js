...
/* These are the lightweight oil presets used by the drift layer. The richer
   ...
   We merge into any pre-existing OIL_TYPES (defensive against script-load
   order — whichever file loaded first wins for shared keys, and the richer
   entries from weathering.js are preserved). */
globalThis.OIL_TYPES = Object.assign({
...
class Drifter {
  constructor(lon, lat, tSec, opts){
    ...
    /* Non-evaporable residue floor — keeps the drifter mass_frac consistent
       with OilBudget's Fingas f_max cap. Without this, the exponential decay
       drives mass_frac → 0 while OilBudget correctly holds a few percent
       residue, producing an order-of-magnitude disagreement at t ~ 3 τ. */
    this.residueFrac = opts.residueFrac ?? 0;
    ...
    // Symmetric ±15% (3-σ ≈ ±45%) — clamp both tails so a single Drifter
    // can't end up with 1.5× nominal leeway from a fat-tail randn() draw.
  ...
  step(dt){
    ...
    const latMid = this.lat + (k1.v * dt/2) / mPerDegLat(this.lat);
    ...
    /* Use the midpoint latitude for the deg-per-metre conversion on the full
       step so the integrator stays consistent at the trapezoidal level. */
    let dLat = (k2.v * dt) / mPerDegLat(latMid);
    let dLon = (k2.u * dt) / mPerDegLon(latMid);
    ...
    dLat += randn() * sigma / mPerDegLat(latMid);
    dLon += randn() * sigma / mPerDegLon(latMid);
    ...
    /* Track remaining oil fraction per particle only. Bulk oil geometry is
       handled elsewhere by the slick/budget helpers. The residueFrac floor
       keeps the per-particle decay consistent with OilBudget's Fingas
       cap so the two layers don't disagree by an order of magnitude. */
    if (this.tau_evap){
      const evap = 1 - Math.exp(-this.age / this.tau_evap);
      const maxEvap = 1 - this.residueFrac;
      this.mass_frac = 1 - Math.min(maxEvap, evap);
    }
...
function spawnEnsemble({lon, lat, tSec, n, scenario, params}){
  ...
  let dw = 0, cw = 0, tau_evap = null, residueFrac = 0;
  if (scenario === 'leeway'){
    ...
    /* Match OilBudget Fingas cap so per-particle mass_frac never decays
       below the published non-evaporable residue. */
    if (typeof oil.f_max === 'number') residueFrac = 1 - oil.f_max / 100;
  }

  for (let k = 0; k < n; k++){
    /* `radius_m` is the σ of the Gaussian release cloud, not a hard radius.
       Roughly 68% of particles fall within radius_m of the release point;
       only ~5% exceed 2× radius_m. */
...
globalThis.spawnEnsemble = spawnEnsemble;
